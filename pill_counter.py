...
def illum_norm(gray):
    """
    Flatten slow lighting gradients. Uses a size-aware sigma so tiny images
    don't get over-smoothed and large images get enough smoothing.
    """
    h, w = gray.shape[:2]
    min_dim = min(h, w)
    # Estimate sigma from size (clip to [7, 45])
    sigma = int(max(7, min(45, round(min_dim / 24))))
...
def _adaptive_block_size(gray):
    """
    Pick an odd block size based on image size to keep the threshold locally meaningful.
    """
    h, w = gray.shape[:2]
    min_dim = min(h, w)
    # Roughly 1/30 of min dimension, then force odd and within [15, 75]
    bs = int(max(15, min(75, round(min_dim / 30))))
    if bs % 2 == 0:
        bs += 1
    return bs
...
def mask_adaptive(gray, invert):
    ...
    m = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, typ, block_size, 10)
    return morph(m)
...
def _safe_open_image(file):
    """
    Open an uploaded/camera image safely, normalize to RGB, and fix EXIF orientation.
    """
    try:
        pil = Image.open(file)
        pil = ImageOps.exif_transpose(pil)   # fix rotated camera images
        pil = pil.convert("RGB")
        return pil
